...
func main() {
	...
	messageService := services.NewMessageService(messageRepo, conversationRepo, userRepo)
	channelService := services.NewChannelService(channelRepo, orgRepo, cfg)
	channelService.StartTokenRefresher() // Start background token refresher
	teamRepo := postgres.NewTeamRepository(db)
	teamService := services.NewTeamService(teamRepo)
	contactRepo := postgres.NewContactRepository(db)
	contactService := services.NewContactService(contactRepo)
	cannedRepo := postgres.NewCannedResponseRepository(db)
	cannedService := services.NewCannedResponseService(cannedRepo)
	...
	aiService, err := services.NewAIService(aiRepo, cfg) // Init AI Service
	if err != nil {
